...
let run test_case =
  ...
  Fiber.both test_case
    (fun () ->
       Switch.run @@ fun sw ->
       connection_handler ~sw (socket :> <Eio.Flow.two_way; Eio.Flow.close>) (`Unix "test-socket")
    );;
```
...
+             "\r\n"
ocaml-mdx-test: [INFO] unix:test-socket: GET /
...
+             "\r\n"
ocaml-mdx-test: [INFO] unix:test-socket: GET /missing
+socket: wrote "HTTP/1.1 404 Not Found\r\n"
+              "\r\n"
- : unit = ()
```

Streaming a response: XXX should do two writes here!
